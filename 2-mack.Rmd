...
See the second example of Mack's 1993 paper

# Mack's model

```{r}
# write.csv(cum2incr(Mortgage),"./tables/Mortgage.csv")
(Mortgage_mack <- MackChainLadder(Mortgage/1000,est.sigma = "Mack"))
Mortgage_mack$sigma^2
Mortgage_mack$f
I<-length(Mortgage_mack$f)
disp_mack<-1/(Mortgage_mack$f[1:(I-1)]-1)*Mortgage_mack$sigma^2
(
  mack_acc <-
    data.frame(
      mean = summary(Mortgage_mack)$ByOrigin$IBNR[-1],
      est_err = Mortgage_mack$Mack.ParameterRisk[-1, I],
      pro_err = Mortgage_mack$Mack.ProcessRisk[-1, I],
      pre_err = Mortgage_mack$Mack.S.E[-1, I],
      cv = Mortgage_mack$Mack.S.E[-1, I]/summary(Mortgage_mack)$ByOrigin$IBNR[-1]
    )
)
# write.csv(mack_acc,"./tables/2_mack.csv")
```
...
## ODP/DP model
```{r}
data_mat<-as.LongTriangle(cum2incr(Mortgage))
...
## residual plot
# pdf("plots/residuals_odp_mortgage.pdf",height =textwidth,width=textwidth,pointsize =11)
...
## residual plot
# pdf("plots/residuals_dp_mortgage.pdf",height =textwidth,width=textwidth,pointsize =11)
par(mfrow=c(1,1),pty="s",mar=c(5,5,2,5))
...
# pdf("plots/disp_mortgage.pdf",height =textwidth,width=textwidth,pointsize =11)
par(mfrow=c(1,1),pty="s",mar=c(5,5,2,5))
...
# write.csv(results_acc_odp,"./tables/2_odp.csv")
# write.csv(results_acc_dp,"./tables/2_dp.csv")
# write.csv(results_acc_ev,"./tables/2_ev.csv")
```
...
# pdf("plots/acc_mean2.pdf",height =textwidth,width=textwidth,pointsize =11)
par(mfrow=c(1,1),pty="s",mar=c(5,5,2,5))
...
# pdf("plots/acc_pre2.pdf",height =textwidth,width=textwidth,pointsize =11)
par(mfrow=c(1,1),pty="s",mar=c(5,5,2,5))
...
# pdf("plots/acc_est2.pdf",height =textwidth,width=textwidth,pointsize =11)
par(mfrow=c(1,1),pty="s",mar=c(5,5,2,5))
...
# pdf("plots/acc_pro2.pdf",height =textwidth,width=textwidth,pointsize =11)
par(mfrow=c(1,1),pty="s",mar=c(5,5,2,5))
...
### Fisher information: not suitable for small sample size.
...
# pdf("plots/2-mack-total.pdf",height =textwidth,width=textwidth,pointsize =11)
par(mfrow=c(1,1),pty="s",mar=c(5,5,2,5))
